    ...
    <div className="absolute inset-0 z-20 flex flex-col bg-gray-900/98 backdrop-blur-sm">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button
            onClick={() => { setMode('+1/+1'); setSelected([]) }}
            className={`px-3 py-1.5 text-xs rounded border transition-colors ${
              mode === '+1/+1'
                ? 'bg-amber-900/30 border-amber-500/50 text-amber-300'
                : 'bg-gray-800 border-gray-700 text-gray-400 hover:text-gray-200'
            ...
            return (
              ...
              >
                ...
                <div className="text-lg font-bold text-gray-100">
                  {score}
                  {boost > 0 && (
                    <span className="text-green-400 text-sm ml-1">+{boost}</span>
                  )}
                </div>
                <div className="text-amber-400 font-bold text-sm">
                  ...
                  {boost > 0 && (
                    <span className="text-green-400 ml-1">
                      ({formatMod(abilityModifier(score + boost))})
                    </span>
                  )}
                </div>
              </button>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-gray-700 bg-gray-900">
        ...
        <div className="flex gap-2">
          ...
          {isAlreadyConfirmed ? (
            <button
              onClick={() => { if (activeAsiSlotId) resetAsi(activeAsiSlotId) }}
