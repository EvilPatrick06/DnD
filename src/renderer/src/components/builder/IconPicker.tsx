...
import { useBuilderStore, PRESET_ICONS } from '../../stores/useBuilderStore'
...
export function CharacterIcon({
  ...
  if (iconType === 'preset' && iconPreset) {
    ...
    return (
      <div className={`${sizeClasses[size]} rounded bg-gray-800 border border-gray-600 flex items-center justify-center shrink-0`}>
        {preset?.emoji ?? '?'}
      </div>
    )
  }
  ...
  return (
    <div className={`${sizeClasses[size]} rounded bg-gray-800 border border-gray-600 flex items-center justify-center text-amber-400 font-bold shrink-0`}>
...
export function getCharacterIconProps(character: {
  name: string
  iconPreset?: string
  portraitPath?: string
}): {
