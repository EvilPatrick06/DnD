...
export default function MechanicsModals({
  ...
  return (
    <Suspense fallback={null}>
      {activeModal === 'item' && (
        <ItemModal character={character} onClose={close} onUseItem={(_itemName, message) => broadcast(message)} />
      )}
      ...
      {/* Companion modals: Familiar, Wild Shape, Steed */}
          ...
          onResummon={() => {
            ...
            if (fam) {
              ...
              if (latest && is5eCharacter(latest)) {
                const updated = {
                  ...latest,
                  companions: (latest.companions ?? []).map((c) => (c.id === fam.id ? { ...c, dismissed: false } : c)),
                  updatedAt: new Date().toISOString()
                }
                useCharacterStore.getState().saveCharacter(updated)
              }
          ...
          onDismiss={() => {
            ...
            if (steed) {
              ...
              if (latest && is5eCharacter(latest)) {
                const updated = {
                  ...latest,
                  companions: (latest.companions ?? []).map((c) => (c.id === steed.id ? { ...c, dismissed: true } : c)),
                  updatedAt: new Date().toISOString()
                }
                useCharacterStore.getState().saveCharacter(updated)
              }
