import { useState, useEffect } from 'react'
import type { GameSystem } from '../../../types/game-system'

interface ActionBarProps {
  system: GameSystem
  isMyTurn: boolean
  onAction?: (action: string) => void
}

interface Actions5e {
  action: boolean
  bonusAction: boolean
  reaction: boolean
}

// D&D 5e common actions
const ACTIONS_5E = [
  { id: 'attack', label: 'Attack', type: 'action' as const },
  { id: 'cast-spell', label: 'Cast Spell', type: 'action' as const },
  { id: 'dash', label: 'Dash', type: 'action' as const },
  { id: 'dodge', label: 'Dodge', type: 'action' as const },
  { id: 'help', label: 'Help', type: 'action' as const },
  { id: 'hide', label: 'Hide', type: 'action' as const },
  { id: 'ready', label: 'Ready', type: 'action' as const },
  { id: 'search', label: 'Search', type: 'action' as const },
  { id: 'use-object', label: 'Use Object', type: 'action' as const }
]

// PF2e action costs
const ACTIONS_PF2E = [
  { id: 'strike', label: 'Strike', cost: 1 },
  { id: 'cast-spell', label: 'Cast Spell', cost: 2 },
  { id: 'move', label: 'Stride', cost: 1 },
  { id: 'step', label: 'Step', cost: 1 },
  { id: 'raise-shield', label: 'Raise Shield', cost: 1 },
  { id: 'seek', label: 'Seek', cost: 1 },
  { id: 'interact', label: 'Interact', cost: 1 },
  { id: 'recall', label: 'Recall Knowledge', cost: 1 },
  { id: 'demoralize', label: 'Demoralize', cost: 1 }
]

export default function ActionBar({
  system,
  isMyTurn,
  onAction
}: ActionBarProps): JSX.Element {
  // 5e state
  const [actions5e, setActions5e] = useState<Actions5e>({
    action: false,
    bonusAction: false,
    reaction: false
  })

  // PF2e state
  const [actionsRemaining, setActionsRemaining] = useState(3)
  const [reactionAvailable, setReactionAvailable] = useState(true)

  // Reset on new turn
  useEffect(() => {
    if (isMyTurn) {
      setActions5e({ action: false, bonusAction: false, reaction: false })
      setActionsRemaining(3)
      setReactionAvailable(true)
    }
  }, [isMyTurn])

  if (system === 'dnd5e') {
    const handleAction5e = (actionId: string, type: string): void => {
      if (type === 'action' && actions5e.action) return
      if (type === 'bonusAction' && actions5e.bonusAction) return
      if (type === 'reaction' && actions5e.reaction) return

      setActions5e((prev) => ({
        ...prev,
        [type]: true
      }))
      onAction?.(actionId)
    }

    return (
      <div className="flex items-center gap-3">
        {/* Action economy indicators */}
        <div className="flex items-center gap-2 pr-3 border-r border-gray-700">
          <div className="flex flex-col items-center">
            <div
              className={`w-4 h-4 rounded-full border-2 ${
                actions5e.action
                  ? 'bg-gray-600 border-gray-600'
                  : 'bg-amber-600 border-amber-500'
              }`}
            />
            <span className="text-[9px] text-gray-500 mt-0.5">Act</span>
          </div>
          <div className="flex flex-col items-center">
            <div
              className={`w-4 h-4 rounded-full border-2 ${
                actions5e.bonusAction
                  ? 'bg-gray-600 border-gray-600'
                  : 'bg-green-600 border-green-500'
              }`}
            />
            <span className="text-[9px] text-gray-500 mt-0.5">Bonus</span>
          </div>
          <div className="flex flex-col items-center">
            <div
              className={`w-4 h-4 rounded-full border-2 ${
                actions5e.reaction
                  ? 'bg-gray-600 border-gray-600'
                  : 'bg-blue-600 border-blue-500'
              }`}
            />
            <span className="text-[9px] text-gray-500 mt-0.5">React</span>
          </div>
        </div>

        {/* Action buttons */}
        <div className="flex gap-1 flex-wrap">
          {ACTIONS_5E.map((action) => {
            const spent = action.type === 'action' && actions5e.action
            return (
              <button
                key={action.id}
                onClick={() => handleAction5e(action.id, action.type)}
                disabled={spent || !isMyTurn}
                className={`px-2.5 py-1 text-xs rounded-lg transition-colors cursor-pointer
                  ${
                    spent
                      ? 'bg-gray-800 text-gray-600 cursor-not-allowed'
                      : 'bg-gray-800 text-gray-300 hover:bg-amber-600 hover:text-white'
                  }
                  disabled:opacity-50 disabled:cursor-not-allowed`}
              >
                {action.label}
              </button>
            )
          })}
        </div>
      </div>
    )
  }

  // PF2e action bar
  const handleActionPf2e = (actionId: string, cost: number): void => {
    if (actionsRemaining < cost) return
    setActionsRemaining((prev) => prev - cost)
    onAction?.(actionId)
  }

  return (
    <div className="flex items-center gap-3">
      {/* Action economy indicators */}
      <div className="flex items-center gap-2 pr-3 border-r border-gray-700">
        <div className="flex gap-1">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className={`w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] font-bold ${
                i <= actionsRemaining
                  ? 'bg-amber-600 border-amber-500 text-white'
                  : 'bg-gray-700 border-gray-600 text-gray-500'
              }`}
            >
              {i <= actionsRemaining ? '\u25C6' : ''}
            </div>
          ))}
        </div>
        <div className="flex flex-col items-center">
          <div
            className={`w-4 h-4 rounded-full border-2 ${
              reactionAvailable
                ? 'bg-blue-600 border-blue-500'
                : 'bg-gray-600 border-gray-600'
            }`}
          />
          <span className="text-[9px] text-gray-500 mt-0.5">React</span>
        </div>
      </div>

      {/* Action buttons */}
      <div className="flex gap-1 flex-wrap">
        {ACTIONS_PF2E.map((action) => {
          const canAfford = actionsRemaining >= action.cost
          return (
            <button
              key={action.id}
              onClick={() => handleActionPf2e(action.id, action.cost)}
              disabled={!canAfford || !isMyTurn}
              className={`px-2.5 py-1 text-xs rounded-lg transition-colors cursor-pointer
                ${
                  !canAfford
                    ? 'bg-gray-800 text-gray-600 cursor-not-allowed'
                    : 'bg-gray-800 text-gray-300 hover:bg-amber-600 hover:text-white'
                }
                disabled:opacity-50 disabled:cursor-not-allowed`}
            >
              <span>{action.label}</span>
              <span className="ml-1 text-[9px] text-gray-500">
                {'\u25C6'.repeat(action.cost)}
              </span>
            </button>
          )
        })}
      </div>
    </div>
  )
}
